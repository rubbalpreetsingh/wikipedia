...
class Wikipedia {
  ...
  Future<WikipediaResponse?> searchQuery(
    ...
    try {
      final _responseData = await http.get(Uri.parse(
          "$_baseUrl?action=query&format=json&list=search&srlimit=$limit&srsearch=$searchQuery"));
  ...
  Future<WikipediaSummaryData?> searchSummaryWithPageId(
    ...
    try {
      final _responseData = await http.get(Uri.parse(
          "$_baseUrl?action=query&format=json&pageids=$pageId&prop=extracts|description"));
